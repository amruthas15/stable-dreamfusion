...
class NeRFRenderer(nn.Module):
    ...
    def run(self, image_coord, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        # image_coord: [B, N, 2], assumes B == 1
        # bg_color: [BN, 3] in range [0, 1]
        # return: image: [B, N, 3], depth: [B, N]

        prefix = image_coord.shape[:-1]
        image_coord = image_coord.contiguous().view(-1, 2)
 
        N = image_coord.shape[0] # N = B * N, in fact
        device = image_coord.device

        results = {}

        rgbs = self(image_coord.reshape(-1, 2))
        rgbs = rgbs.view(N, -1, 3) # [N, T+t, 3]

        # calculate color
        image = torch.sum(rgbs, dim=-2) # [N, 3], in [0, 1]
    ...
    def render(self, image_coord, staged=False, max_ray_batch=4096, **kwargs):
        # image_coord: [B, N, 2], assumes B == 1
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    results_ = _run(image_coord[b:b+1, head:tail], **kwargs)
            ...
            results = _run(image_coord, **kwargs)
